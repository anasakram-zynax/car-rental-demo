  ...
  return (
    <aside className="border-b border-white/10 bg-[#101d37] p-3 text-white shadow-[0_18px_50px_rgba(16,29,55,0.18)] lg:sticky lg:top-0 lg:min-h-svh lg:self-start lg:border-r lg:border-b-0 lg:p-5">
      <div className="flex items-center justify-between gap-4 lg:block">
        ...
        >
          <span className="grid size-9 place-items-center rounded-control bg-blue-500 text-white shadow-lg shadow-blue-950/30"><CarFront aria-hidden="true" size={19} /></span>
          <span><span className="block text-sm">Northstar</span><span className="block text-[10px] font-medium tracking-[0.16em] text-slate-400 uppercase">Administration</span></span>
        </Link>
        <nav aria-label="Admin navigation" className="lg:mt-8">
          <ul className="flex flex-wrap gap-1 lg:grid">
            {adminNavigation.map(({ href, icon: Icon, label }) => {
              const isActive = href === "/admin" ? pathname === href : href.startsWith("/admin") && pathname.startsWith(href);
              return (
              <li key={href}>
                <Link
                  href={href}
                  ...
                  className={cn(
                    ...
                    href === "/" && "lg:mt-4 lg:border-t lg:border-white/10 lg:pt-4",
                  )}
                >
                  <Icon aria-hidden="true" className={isActive ? "text-blue-300" : "text-slate-400"} size={18} />
