...
export function useSaveAdminCar() {
  ...
  return useMutation({
    ...
    mutationFn: async ({
      ...
      carId?: string;
    }): Promise<AdminCarSaveResult> => {
      let car = carId
        ? await updateCar(carId, input)
        : await createCar(input as CreateCarInput);
      ...
      try {
        ...
        for (const imageId of removedImageIds) {
          ...
          if (images.length && !images.some((image) => image.isDefault)) {
            const nextId = [...images].sort((a, b) =>
              a.id.localeCompare(b.id),
            )[0].id;
            images.forEach((image) => {
              image.isDefault = image.id === nextId;
            });
          }
          car = { ...car, images };
        }
        ...
        return {
          ...
          remainingFiles,
          remainingRemovalIds,
          warning: `Car ${carId ? "saved" : "created"}, but image changes could not be completed. ${detail} Review images and save again to retry. The car has been kept.`,
        };
      }
    },
    onSettled: async () => {
