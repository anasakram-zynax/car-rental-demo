...
export function useUpdateCarStatus() {
  const invalidateAdminCars = useInvalidateAdminCars();
  return useMutation({ mutationFn: ({ id, status }: { id: string; status: "active" | "inactive" }) => updateCar(id, { status }), retry: false, onSuccess: invalidateAdminCars });
}

export function useCreateCar() {
  const invalidateAdminCars = useInvalidateAdminCars();
  return useMutation({ mutationFn: (input: CreateCarInput) => createCar(input), retry: false, onSuccess: invalidateAdminCars });
}

export function useUpdateCar() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, input }: { id: string; input: UpdateCarInput }) => updateCar(id, input),
    retry: false,
    onSuccess: (_, { id }) => Promise.all([
      queryClient.invalidateQueries({ queryKey: ["admin", "cars", "detail", id] }),
