    ...
    <>
      <Card padding="lg" className="overflow-hidden">
        ...
        <div className="mt-6 grid gap-5 border-y border-border py-5 sm:grid-cols-2">
          {isTransfer ? <div className="flex gap-3"><MapPin aria-hidden="true" className="mt-0.5 shrink-0 text-accent-secondary" size={18} /><div><p className="text-xs font-medium text-muted">Transfer route</p><p className="mt-1 break-words text-sm font-semibold">{transferRoute ?? "Package details unavailable"}</p></div></div> : <div className="flex gap-3"><MapPin aria-hidden="true" className="mt-0.5 shrink-0 text-accent-secondary" size={18} /><div><p className="text-xs font-medium text-muted">Pickup</p><p className="mt-1 break-words text-sm font-semibold">{booking.pickupLocation}</p><p className="mt-1 text-sm text-muted">{formatDateTime(booking.pickupAt)}</p></div></div>}
          {isTransfer ? <div className="flex gap-3"><CalendarRange aria-hidden="true" className="mt-0.5 shrink-0 text-accent-secondary" size={18} /><div><p className="text-xs font-medium text-muted">Pickup time</p><p className="mt-1 text-sm font-semibold">{formatDateTime(booking.pickupAt)}</p></div></div> : <div className="flex gap-3"><CalendarRange aria-hidden="true" className="mt-0.5 shrink-0 text-accent-secondary" size={18} /><div><p className="text-xs font-medium text-muted">Return</p><p className="mt-1 break-words text-sm font-semibold">{booking.dropoffLocation}</p><p className="mt-1 text-sm text-muted">{formatDateTime(booking.returnAt)}</p></div></div>}
        </div>
        {isTransfer ? <dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2"><div><dt className="text-muted">Package price</dt><dd className="mt-1 font-semibold">{booking.transferPackage ? `${formatCurrency(booking.transferPackage.price, booking.transferPackage.currency)} ${booking.transferPackage.currency}` : "Package details unavailable"}</dd></div></dl> : <dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-4"><div><dt className="text-muted">Rental days</dt><dd className="mt-1 font-semibold">{booking.rentalDays}</dd></div><div><dt className="text-muted">Daily rate</dt><dd className="mt-1 font-semibold">{formatCurrency(booking.dailyPrice, booking.currency)}</dd></div><div><dt className="text-muted">Tax</dt><dd className="mt-1 font-semibold">{formatCurrency(booking.taxAmount, booking.currency)}</dd></div><div><dt className="text-muted">Total</dt><dd className="mt-1 font-semibold">{formatCurrency(booking.totalPrice, booking.currency)}</dd></div></dl>}
        <div className="mt-6 grid gap-4 border-t border-border pt-5 text-sm sm:grid-cols-2"><p><span className="text-muted">{isTransfer ? "Passenger" : "Driver"}</span><br /><strong>{booking.driverFirstName} {booking.driverLastName}</strong></p><p className="break-words"><span className="text-muted">Contact</span><br /><strong>{booking.contactEmail}</strong><br />{booking.contactPhone}</p>{booking.specialRequests ? <p className="sm:col-span-2"><span className="text-muted">Special requests</span><br />{booking.specialRequests}</p> : null}{booking.cancelReason ? <p className="sm:col-span-2"><span className="text-muted">Cancellation reason</span><br />{booking.cancelReason}</p> : null}</div>
