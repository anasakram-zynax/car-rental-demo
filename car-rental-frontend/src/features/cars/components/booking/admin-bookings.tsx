...
import {
  BookingStatusBadge,
  PaymentStatusBadge,
} from "@/features/cars/components/booking/booking-status-badge";
import {
  useAdminBooking,
  useAdminBookings,
  useUpdateAdminBookingPaymentStatus,
} from "@/features/cars/hooks/use-admin-bookings";
import type {
  CarBooking,
  PaymentStatus,
...
function ErrorNotice({ error }: { error: unknown }) {
  return (
    <div
      role="alert"
      className="flex items-start gap-3 rounded-control border border-danger/20 bg-red-900/[0.06] px-4 py-3 text-sm text-danger"
    >
...
function BookingSkeleton() {
  return (
    <div className="space-y-3 p-5">
      <div className="h-5 w-1/3 animate-pulse rounded bg-black/[0.08]" />
      <div className="h-14 animate-pulse rounded bg-black/[0.06]" />
      <div className="h-14 animate-pulse rounded bg-black/[0.06]" />
    </div>
  );
}
...
export function AdminBookings() {
  ...
  const bookings = useMemo(() => {
    ...
    return (query.data ?? []).filter((booking) =>
      `${booking.reference} ${booking.driverFirstName} ${booking.driverLastName} ${booking.contactEmail}`
        .toLowerCase()
        .includes(term),
    );
  }, [query.data, search]);
    ...
    <PageContainer className="py-8 sm:py-10 lg:py-12">
      <div>
        ...
        <p className="mt-3 text-muted">
          Review reservations and manage payment status.
        </p>
      </div>
      <div className="relative mt-8 max-w-lg rounded-card border border-border bg-white p-4 shadow-card">
        <Input
          label="Search bookings"
          placeholder="Reference, driver, or email"
          value={search}
      ...
      >
        <div className="flex items-center justify-between border-b border-border bg-[#f8fafd] px-5 py-4 sm:px-6">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">
              {query.data
                ? `${bookings.length} of ${query.data.length} bookings`
                : "Loading bookings..."}
            </p>
          </div>
        </div>
        {query.isPending ? <BookingSkeleton /> : null}
        {query.isError ? (
          <div className="p-5">
        ...
        {query.data && !query.isError ? (
          bookings.length ? (
            ...
            <EmptyBookings hasSearch={Boolean(search.trim())} />
          )
        ) : null}
      </Surface>
    </PageContainer>
...
function BookingTableRow({ booking }: { booking: CarBooking }) {
  const isTransfer = isTransferBooking(booking);
  const route = getTransferPackageRoute(booking);
  return (
    <tr className="transition-colors hover:bg-blue-50/35">
      <td className="px-6 py-4 font-semibold">{booking.reference}</td>
      <td className="px-4 py-4">
        <p className="font-medium">
          {booking.driverFirstName} {booking.driverLastName}
        </p>
        <p className="mt-1 text-xs text-muted">{booking.contactEmail}</p>
      </td>
      <td className="px-4 py-4">
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant={isTransfer ? "accent" : "neutral"}>
            {isTransfer ? "Transfer" : "Rental"}
          </Badge>
          <p>
            {isTransfer
              ? (route ?? "Package details unavailable")
      ...
      <td className="px-6 py-4 text-right">
        ...
        >
          View <ChevronRight aria-hidden="true" size={15} />
        </Link>
      </td>
    </tr>
  );
}
function BookingMobileCard({ booking }: { booking: CarBooking }) {
  const isTransfer = isTransferBooking(booking);
  const route = getTransferPackageRoute(booking);
  return (
    <article className="p-5 transition-colors hover:bg-blue-50/30 sm:p-6">
        ...
        {isTransfer ? (
          <>
            <p>
              ...
              <br />
              <strong>{route ?? "Package details unavailable"}</strong>
            </p>
            <p>
              <span className="text-muted">Pickup</span>
              <br />
              <strong>{formatDateTime(booking.pickupAt)}</strong>
            </p>
            <p>
      ...
      >
        View booking <ChevronRight aria-hidden="true" size={16} />
      </Link>
    </article>
  );
}
function EmptyBookings({ hasSearch }: { hasSearch: boolean }) {
  return (
    <div className="grid min-h-56 place-items-center p-6 text-center">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">
          {hasSearch
            ? "Try another reference, name, or email."
            : "Bookings will appear here when customers reserve a car."}
        </p>
      </div>
    </div>
  );
...
): Extract<PaymentStatus, "paid" | "refunded"> | null {
  return status === "unpaid" ? "paid" : status === "paid" ? "refunded" : null;
}
function PaymentControl({ booking }: { booking: CarBooking }) {
  const mutation = useUpdateAdminBookingPaymentStatus();
  const [confirmStatus, setConfirmStatus] = useState<Extract<
    PaymentStatus,
    "paid" | "refunded"
  > | null>(null);
  const [error, setError] = useState<string | null>(null);
  const target = nextPaymentStatus(booking.paymentStatus);
  ...
  if (!target) return <PaymentStatusBadge status={booking.paymentStatus} />;
  const label = target === "paid" ? "Mark as Paid" : "Mark as Refunded";
  return (
    <>
      <Button
      ...
      {confirmStatus ? (
        <div
          ...
          onMouseDown={() => setConfirmStatus(null)}
        >
          <section
            role="dialog"
            aria-modal="true"
          ...
          >
            <h2 id="payment-confirm-title" className="text-xl font-semibold">
              {confirmStatus === "paid"
                ? "Mark this booking as paid?"
                : "Mark this payment as refunded?"}
            </h2>
            <p className="mt-3 text-sm leading-6 text-muted">
              Booking: {booking.reference}. This payment transition cannot be
...
export function AdminBookingDetail({ reference }: { reference: string }) {
    ...
    return (
      <DetailShell>
        <div className="space-y-5">
          <div className="h-36 animate-pulse rounded-card bg-black/[0.07]" />
          <div className="h-64 animate-pulse rounded-card bg-black/[0.07]" />
        </div>
      </DetailShell>
    );
      ...
      <DetailShell>
        <Surface variant="elevated" padding="lg" className="max-w-xl">
          <h1 className="text-2xl font-semibold">
            {notFound ? "Booking not found" : "Unable to load booking"}
          </h1>
          <p className="mt-3 text-muted">
            {notFound
              ? "This booking reference does not exist."
  ...
  const journeyItems: [string, string | null][] = isTransfer
    ...
    : [
        ...
        [
          "Pickup",
          `${booking.pickupLocation} · ${formatDateTime(booking.pickupAt)}`,
        ],
        [
          "Return",
          `${booking.dropoffLocation} · ${formatDateTime(booking.returnAt)}`,
        ],
    ...
    <DetailShell>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <div className="space-y-6">
          <BookingSection
            title={isTransfer ? "Transfer details" : "Rental details"}
            icon={<MapPin aria-hidden="true" size={19} />}
          >
            <Details items={journeyItems} />
          </BookingSection>
          <BookingSection
            title={isTransfer ? "Passenger" : "Driver"}
            icon={<CreditCard aria-hidden="true" size={19} />}
          >
            <Details items={personItems} />
          </BookingSection>
          {booking.specialRequests || booking.cancelReason ? (
            <BookingSection title="Other">
              <Details
                items={[
...
function DetailShell({ children }: { children: React.ReactNode }) {
  return (
    <PageContainer className="py-8 sm:py-10 lg:py-12">
      ...
      >
        <ArrowLeft aria-hidden="true" size={16} /> Back to Bookings
      </Link>
...
function BookingSection({
  ...
  return (
    <Surface variant="elevated" padding="lg" className="border-white bg-white">
...
function Details({ items }: { items: [string, string | null][] }) {
  return (
