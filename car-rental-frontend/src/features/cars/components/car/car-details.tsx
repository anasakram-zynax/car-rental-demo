        ...
        {isTransfer ? (
          <ul className="mt-5 space-y-3">
            {car.transferPackages.map((item) => (
              <li
                key={item.id}
                className="rounded-control border border-border bg-white px-4 py-3.5 text-sm shadow-sm"
              >
                <div className="flex items-start justify-between gap-3">
                  <p className="font-semibold leading-5">
                    {item.fromLocation} <span className="text-accent">→</span>{" "}
                    {item.toLocation}
                  </p>
                  <Waypoints
                    aria-hidden="true"
        ...
        <div className="mt-5 grid gap-2.5 border-t border-border pt-5 text-xs leading-5 text-muted">
          <p className="flex gap-2">
            <ShieldCheck
              ...
              size={15}
            />{" "}
            Availability is checked when you book.
          </p>
          <p className="flex gap-2">
            <CircleDollarSign
              aria-hidden="true"
            ...
            />{" "}
            {isTransfer
              ? "Routes use their listed fixed price."
              : "Your date-based estimate is shown before confirmation."}
          </p>
        </div>
      </div>
    </Card>
...
export function CarDetails({ carId }: CarDetailsProps) {
  ...
  if (carQuery.isError)
    return <QueryError error={carQuery.error} onRetry={retry} />;
  if (!carQuery.data)
    return (
      <QueryError error={new ApiError("Car not found", 404)} onRetry={retry} />
    );
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <Badge className="border-blue-200 bg-blue-50 text-accent">
              {formatServiceType(car.serviceType)}
            </Badge>
            {car.withDriver ? (
              <Badge variant="success">With Driver</Badge>
            ) : null}
            {car.featured ? (
          ...
          <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-3 text-base text-muted sm:text-lg">
                {car.brand} {car.model} <span aria-hidden="true">•</span>{" "}
                {car.year}
              </p>
            </div>
          ...
          <Card className="mt-7" padding="lg" variant="elevated">
            ...
            <dl className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {specifications(car).map(({ icon: Icon, label, value }) => (
                <div
                  key={label}
                  className="flex items-center gap-3 rounded-control border border-border/80 bg-[#f8fafe] px-3.5 py-3"
                >
