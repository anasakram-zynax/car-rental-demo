...
import { useSaveAdminCar, type AdminCarSubmission } from "@/features/cars/hooks/use-save-admin-car";
...
function messageFor(error: unknown) {
  return error instanceof ApiError ? error.message : "Unable to save this car. Please try again.";
}
...
export function AdminCreateCarScreen() {
  ...
  async function submit(submission: AdminCarSubmission) {
    ...
    try {
      const result = await saveCar.mutateAsync({ ...submission, carId: createdCar?.id });
      if (result.warning) {
  ...
  return <EditorShell eyebrow="Fleet management" title={createdCar ? `Edit ${createdCar.name}` : "Add a car"}><p className="mb-6 max-w-2xl text-muted">Add vehicle details and upload images to the fleet catalog.</p><AdminCarForm car={createdCar} pending={saveCar.isPending} submitLabel={createdCar ? "Retry image changes" : "Create car"} submissionError={submissionError} onCancel={() => router.push("/admin/cars")} onSubmit={submit} /></EditorShell>;
}
...
export function AdminEditCarScreen({ carId }: { carId: string }) {
  ...
  if (carQuery.isError || !carQuery.data) {
    ...
    return <EditorShell eyebrow="Fleet management" title={notFound ? "Car not found" : "Unable to load car"}><Surface variant="elevated" padding="lg" className="max-w-xl"><p className="text-muted">{notFound ? "This car is no longer available in the admin catalog." : "Please check your connection and try again."}</p>{!notFound ? <Button className="mt-6" onClick={() => void carQuery.refetch()}>Try Again</Button> : null}</Surface></EditorShell>;
  }
  return <EditorShell eyebrow="Fleet management" title={`Edit ${carQuery.data.name}`}><p className="mb-6 max-w-2xl text-muted">Update vehicle details, transfer packages, and saved images.</p><AdminCarForm car={carQuery.data} pending={saveCar.isPending} submitLabel="Save changes" submissionError={submissionError} onCancel={() => router.push("/admin/cars")} onSubmit={submit} /></EditorShell>;
}
